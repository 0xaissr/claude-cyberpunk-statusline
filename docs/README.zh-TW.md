...
同時每次狀態變動也會把 tab 標題設為 `<basename> <emoji>`（🟢 running / 🟡 waiting /
🔵 idle / 🔴 error）。emoji 放在**結尾**，因為 iTerm2 的 tab 擠窄時會從**開頭**截斷
（顯示成 `…sline`），把 emoji 放後面才不會被砍掉，tab 再窄都還看得到 state。暗色 palette 會自動拉亮（等比例把最亮 channel 提升到
